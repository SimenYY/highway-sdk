...
class Bytes16:
    ...
    def __init__(self, fcs: Union[int, bytes]):
        if type(fcs) == bytes:
            if len(fcs) != 2:
                raise ValueError("fcs must be 2 bytes")
            else:
                fcs_int = int.from_bytes(fcs, byteorder='big')
                self.__init__(fcs_int)
        elif type(fcs) == int:
...
class CrcUtils:
    ...
    def isCheckSuccess(crc: Bytes16, calc_crc: Bytes16) -> bool:
        ...
        :return:
        """
        if crc.h == calc_crc.h and crc.l == calc_crc.l:
            return True
        else:
            return False
    ...
    def YingSha_crc_16_table(buffer: bytes) -> Bytes16:
        ...
        for i in range(len(buffer)):
            crc = ys_table[((crc >> 8) ^ buffer[i]) & 0xFF] ^ (crc << 8)
