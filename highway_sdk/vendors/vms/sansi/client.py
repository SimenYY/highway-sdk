import asyncio
from highway_sdk.core.driver import AioTCPClient, SequenceDriverTCPClientProtocol
from highway_sdk.core.exceptions import (
    CrcValidationError,
    ConnectionLostError,
    HostResponseTimeoutError,
)
from .spec import SansiRespFrame, SansiMessageFactory, SansiWhat
from .parse import (
    SanSiGetItemParser,
...
class VmsSanSiClient(AioTCPClient):
    ...
    async def request(
        ...
        try:
            frame = SansiRespFrame.unpack(resp)
        except ValueError as e:
            raise CrcValidationError(e)

        return frame

    async def get_item(self, **kwargs) -> SansiRespFrame:
        """获取当前播放项
    ...
    async def get_brightness(self, **kwargs) -> SansiRespFrame:
        """获取亮度
    ...
    async def set_brightness(self, brightness: int, **kwargs) -> SansiRespFrame:
        """设置亮度
...
class VmsSansiClientDriverProtocol(SequenceDriverTCPClientProtocol):
