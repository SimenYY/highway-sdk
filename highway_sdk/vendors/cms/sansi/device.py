...
class SanSiCms(BaseDevice[SanSiCodec]):
    """三思CMS设备客户端。
    ...
    async def get_play_list(self, file_name: str = "play.lst") -> CmsTags:
        """获取当前播放列表（结构化 + 原始格式）。
    ...
    def _item_to_media_list(cls, item: CmsPlayItem) -> list[BaseMedia]:
        ...
        if item.image_name:
            ...
            if ext == "png":
                ...
                media_list.append(Mpg(x=x, y=y, mpg_file_name=item.image_name))
            else:
